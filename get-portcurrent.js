module.exports = function(RED) {
    function GetPortCurrent(config) {  
        RED.nodes.createNode(this, config);
        var node = this;

        let ip = config.ip;
        let port = config.port.toString();
        let url = `http://${ip}/iolink/v1/vendor/masters/1/ports/${port}/diagnostics/current`;
        ...
        node.on('input', function(msg) {
			...
			url = `http://${ip}/iolink/v1/vendor/masters/1/ports/${port}/diagnostics/current`;
    ...
    RED.nodes.registerType("get-portcurrent", GetPortCurrent);
}
