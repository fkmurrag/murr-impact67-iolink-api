module.exports = function(RED) {
    function GetPortStatisticTemperature(config) {  
        RED.nodes.createNode(this, config);
        var node = this;

        let ip = config.ip;
        let port = config.port.toString();
        let url = `http://${ip}/iolink/v1/vendor/masters/1/ports/${port}/statistics/temperature`;
        ...
        node.on('input', function(msg) {
			...
			url = `http://${ip}/iolink/v1/vendor/masters/1/ports/${port}/statistics/temperature`;
    ...
    RED.nodes.registerType("get-porttemperaturestatistic", GetPortStatisticTemperature);
}
