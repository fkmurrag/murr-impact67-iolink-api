module.exports = function(RED) {
    function GetPortStatisticCurrent(config) {  
        RED.nodes.createNode(this, config);
        var node = this;

        let ip = config.ip;
        let port = config.port.toString();
        let url = `http://${ip}/iolink/v1/vendor/masters/1/ports/${port}/statistics/current`;
        ...
        node.on('input', function(msg) {
            ...
            // URL aktualisieren
            url = `http://${ip}/iolink/v1/vendor/masters/1/ports/${port}/statistics/current`;
    ...
    RED.nodes.registerType("get-portcurrentstatistic", GetPortStatisticCurrent);
}
