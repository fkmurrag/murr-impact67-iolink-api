module.exports = function(RED) {
    function GetPortStatisticVoltage(config) {  
        RED.nodes.createNode(this, config);
        var node = this;

        let ip = config.ip;
        let port = config.port.toString();
        let url = `http://${ip}/iolink/v1/vendor/masters/1/ports/${port}/statistics/voltage`;

        const http = require('http');

        node.on('input', function(msg) {
            // Update IP if msg.ip is present and valid
            ...
            // Update port if msg.port is present and valid
            ...
            // Update URL with new IP and port
            url = `http://${ip}/iolink/v1/vendor/masters/1/ports/${port}/statistics/voltage`;
    ...
    RED.nodes.registerType("get-portvoltagestatistic", GetPortStatisticVoltage);
}
