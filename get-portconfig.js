module.exports = function(RED) {
    function GetPortConfig(config) {  
        RED.nodes.createNode(this, config);
        var node = this;

        let ip = config.ip;
        let port = config.port.toString();
        let url = `http://${ip}/iolink/v1/masters/1/ports/${port}/configuration`;
        ...
        node.on('input', function(msg) {
            ...
            url = `http://${ip}/iolink/v1/masters/1/ports/${port}/configuration`;
    ...
    RED.nodes.registerType("get-portconfig", GetPortConfig);
}
